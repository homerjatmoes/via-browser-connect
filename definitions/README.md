...
`mode` is `usb` or `2.4g`. VIA packed id is `(VID << 16) | PID`.
...
Official zips for F75 Ultra, QK108, Galaxy65, and RK61 only included **one** JSON each. I did not invent 2.4G PIDs. If a dongle uses a different PID, add `*_24G.json` and a row here.

Load in VIA: Settings → Show Design tab → Design → Load. Enable **Use V2 definitions** if VIA shows a red error.
