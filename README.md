...
Linux helper that writes udev rules so Chrome can open USB keyboards through WebHID. Use it for [VIA](https://usevia.app), [Vial](https://vial.rocks), and Keychron Launcher.
...
2. Fully quit Chrome and reopen it. Snap Chrome often ignores udev — use a `.deb` or distro package.
...
Those are different products. If Chrome logs `NotAllowedError` on **Wireless 2.4G Dongle**, select that receiver in this script (or allow every hidraw device), then replug the dongle.
...
That is a Linux hidraw permission error, not a VIA JSON problem. Re-run this script so the matching VID:PID is in the rules, replug, restart Chrome. Check `chrome://device-log/` for HID lines.
